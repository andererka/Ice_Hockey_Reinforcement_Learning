...
def main(games_path, id, record, render, output_path, verbose):
    ...
    matches = [
        np.load(match, allow_pickle=True)["arr_0"].item()
        for match in glob(os.path.join(games_path, "**", "*.npz"), recursive=True)
    ]
    if not id is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--render", action="store_true", help="Whether to render in realtime or not"
    )
    parser.add_argument(
        "--id",
        default=None,
        help="id of game you want to replay, do not specify to run all",
    )
