...
def parseOptions():
    parser = argparse.ArgumentParser(
        description="ALRL2021 Competition Client.",
...
class Client:
    ...
    def __init__(
        ...
        self.network_interface = NetworkInterface(
            client=self, server=server_addr, port=server_port
        )
    ...
    def _greeting(self) -> None:
        message = """
       Welcome to the ALRL2020 Competition.
...
"""
    ...
    def connection_error(self, conn_err,) -> None:
    ...
    def step(
        ...
        try:
            self.current_game.add_transition(
                next_obs=ob, next_action=action, r=r, done=done, info=info
            )
    ...
    def game_done(
        self, ob: List[float], r: int, done: int, info: Dict, result: str
    ) -> None:
        ...
        self.current_game.add_transition(
            next_obs=ob, next_action=None, r=r, done=done, info=info
        )
