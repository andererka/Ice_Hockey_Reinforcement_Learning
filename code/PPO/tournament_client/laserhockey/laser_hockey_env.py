...
class LaserHockeyEnv(gym.Env, EzPickle):
    ...
    def _create_player(self, position, color, is_player_two):
        player = self.world.CreateDynamicBody(
            ...
            fixtures=fixtureDef(
                shape=polygonShape(
                    vertices=[
                        (-x / SCALE if is_player_two else x / SCALE, y / SCALE)
    ...
    def _create_world(self):
        ...
        def _create_decoration():
            ...
            objs[-1].color1 = (255.0 / 255, 204.0 / 255, 191.0 / 255)
            objs[-1].color2 = (255.0 / 255, 204.0 / 255, 191.0 / 255)
        ...
        self.world_objects.append(
            _create_wall((W / 2 - 245 / SCALE, H - 52.5 / SCALE - 1), poly)
        )
        self.world_objects.append(
            _create_wall((W / 2 - 245 / SCALE, 52.5 / SCALE + 1), poly)
        )
    ...
    def reset(self, one_starting=None, mode=None):
        ...
        self.time = 0
        if mode is not None and mode in [
            self.NORMAL,
            self.TRAIN_SHOOTING,
            self.TRAIN_DEFENSE,
        ]:
        ...
        # Create players
        self.player1 = self._create_player((W / 5, H / 2), (1, 0, 0), False)
        if self.mode != self.NORMAL:
            ...
            self.player2 = self._create_player((4 * W / 5, H / 2), (0, 0, 1), True)
        if self.mode == self.NORMAL or self.mode == self.TRAIN_SHOOTING:
            ...
            self.puck = self._create_puck(
                (
                    ...
                    H / 2 + 0.9 * self.r_uniform(-H / 2, H / 2),
                ),
                (0, 0, 0),
            )
            force = (
                -(
                    self.puck.position
                    - (0, H / 2 + self.r_uniform(-66 / SCALE, 66 / SCALE))
                )
                * self.puck.mass
                / self.timeStep
            )
    ...
    def _apply_action_with_max_speed(self, player, action, max_speed, is_player_one):
        ...
        if (is_player_one and player.position[0] > CENTER_X) or (
            not is_player_one and player.position[0] < CENTER_X
    ...
    def _get_info(self):
        ...
        if self.puck.position[0] < CENTER_X:
            ...
            max_dist = 10.0
            max_reward = -5.0  # max (negative) reward through this proxy
    ...
    def discrete_to_continous_action(self, discrete_action):
        ...
        """
        action_cont = [
            (discrete_action == 1) * -1 + (discrete_action == 2) * 1,  # player x
            (discrete_action == 3) * -1 + (discrete_action == 4) * 1,  # player y
            (discrete_action == 5) * -1 + (discrete_action == 6) * 1,
        ]  # player angle
