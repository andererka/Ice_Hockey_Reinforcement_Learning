...
# import DDPG.feedforward_ACTOR_CRITIC as AC
...
class PPO:
    def __init__(
        ...
        self.policy = AC.ActorCritic(
            state_dim, action_dim, n_latent_var, action_std
        ).to(self.device)
    ...
    def update(self, memory):
        ...
        _, state_values, _, _ = self.policy.evaluate(
            old_states, old_actions, second_hidden
        )
        ...
        for _ in range(self.K_epochs):
            ...
            logprobs, state_values, dist_entropy, _ = self.policy.evaluate(
                old_states, old_actions, first_hidden
            )
